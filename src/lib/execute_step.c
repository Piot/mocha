...
const mocha_object *mocha_execute_step_exec(const mocha_execute_step *self, const mocha_context *context, const mocha_object *input)
{
	MOCHA_LOG("Execute step '%s' '%s'", self->debug_name, mocha_print_object_debug_str(input));
	const mocha_object *a = self->fn(self->step_state, context, input);
	MOCHA_LOG("execute step '%s' returned %s", self->debug_name, mocha_print_object_debug_str(a));
	return a;
}
