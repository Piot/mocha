...
const struct mocha_object* mocha_transduce_internal(const struct mocha_context* context, mocha_transducer_stepper stepper, const struct mocha_list* arguments)
{
	...
	for (size_t i=0; i<count; ++i) {
		...
		const struct mocha_object* predicate_value = execute_1(context, predicate_fn_object, item);
