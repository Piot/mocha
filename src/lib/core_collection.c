...
MOCHA_FUNCTION(assoc_func)
{
	...
	MOCHA_RESULT_VALUE(result_callback, new_seq_object);
}
...
MOCHA_FUNCTION(dissoc_func)
{
	...
	if (map->count == 0 || arguments->count < 3) {
		MOCHA_RESULT_VALUE(result_callback, map_object);
		return;
	}
	...
	MOCHA_RESULT_VALUE(result_callback, new_map);
}
...
typedef struct for_info {
	...
	resolve_callback result_callback;
} for_info;
...
static void for_next_done(void* user_data, const mocha_object* result)
{
	...
	if (self->iteration_index == self->iteration_count) {
		...
		self->result_callback.callback(self->result_callback.user_data, result_list);
...
static void for_next(for_info* self)
{
	...
	resolve_closure_ex(self->context, self->body, self, for_next_done);
}
...
MOCHA_FUNCTION(for_func)
{
	...
	info->result_callback = result_callback;
...
static void some_next(some_info* self)
{
	...
	execute_resolve_info.callback = some_next_resolved;
...
MOCHA_FUNCTION(empty_func)
{
	...
	MOCHA_RESULT_VALUE(result_callback, is_empty_object);
}
...
MOCHA_FUNCTION(repeat_func)
{
	...
	MOCHA_RESULT_VALUE(result_callback, repeat_list_object);
}
...
MOCHA_FUNCTION(range_func)
{
	...
	MOCHA_RESULT_VALUE(result_callback, list_object);
}
